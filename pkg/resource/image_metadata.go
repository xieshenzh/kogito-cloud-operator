...
// GetImagePrometheusAnnotations retrieves prometheus configurations from the prometheus.io labels of the dockerImage
func GetImagePrometheusAnnotations(dockerImage *dockerv10.DockerImage) (scrape bool, scheme string, path string, port *intstr.IntOrString, err error) {
	if !dockerImageHasLabels(dockerImage) {
		return
	}
