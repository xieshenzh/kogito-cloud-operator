...
func NewServiceMonitor(kogitoApp *v1alpha1.KogitoApp, dockerImage *dockerv10.DockerImage, service *corev1.Service, client *client.Client) (*monv1.ServiceMonitor, error) {
	if ready, err := isPrometheusOperatorReady(kogitoApp, client); err != nil {
		return nil, err
	} else if !ready {
		return nil, nil
	}

	scrape, scheme, path, port, err := resource.GetImagePrometheusAnnotations(dockerImage)
...
func isPrometheusOperatorReady(kogitoApp *v1alpha1.KogitoApp, client *client.Client) (bool, error) {
	log.Debugf("Try to list service monitor instances in the namespace %s and verify if prometheus operator is ready.", kogitoApp.Namespace)

	_, err := prometheus.ServiceMonitorC(client).List(kogitoApp.Namespace)
	if err != nil {
		if errors.IsNotFound(err) {
			log.Debug("Could not find service monitor resources. Prometheus is not ready.")
			return false, nil
		}

		log.Warn("Failed to list service monitor instances. ", err)
		return false, err
	}
