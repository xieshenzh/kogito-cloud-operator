...
func GetKeycloakProperties(cli *client.Client, infra *v1alpha1.KogitoInfra) (name, route string, err error) {
	if keycloak, err := GetKeycloakInstance(infra.Status.Keycloak.Name, infra.Namespace, cli); err != nil {
		...
		len(keycloak.Status.SecondaryResources["Route"]) > 0 {
		return keycloak.Name, keycloak.Status.SecondaryResources["Route"][0], nil
	}
	return "", "", nil
}
